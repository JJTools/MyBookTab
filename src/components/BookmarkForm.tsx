...
export default function BookmarkForm({ bookmark, onSubmit, onCancel, isEdit = false }: BookmarkFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 max-w-lg mx-auto relative cartoon-form-animation animate-bounce-in">
      <button 
        type="button" 
        onClick={clearForm}
      ...
      <div className="flex justify-end space-x-4">
        <button
          type="button"
          onClick={onCancel}
          className="cartoon-button-secondary transition-transform hover:scale-105 hover:-rotate-1 active:scale-95"
          disabled={isLoading}
        >
          <FiXCircle size={18} className="mr-1" />
          {t('common.cancel')}
        </button>
        <button
          type="submit"
          className="cartoon-button transition-transform hover:scale-105 hover:rotate-1 active:scale-95 animate-pulse-slow"
        ...
        >
          {isLoading ? (
            ...
            <span className="flex items-center">
              {isEdit ? <FiCheck size={18} className="mr-1" /> : <FiPlus size={18} className="mr-1 animate-bounce" />}
